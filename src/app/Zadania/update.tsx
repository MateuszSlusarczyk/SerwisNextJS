...
export const ZadaniaUpdate = () => {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    setFormValues({ Nazwa_zadania: '', Nowa_nazwa_zadania:'', id_projektu: '', OdpowiedzialnaOsoba: '', Grafika: null, Opis_zadania: '' });
    try {
      ...
      const res = await fetch('/api/ZadaniaUpdate', {
        method: 'POST',
    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="mb-2">
        <input
          required
          type="text"
          name="Nowa_nazwa_zadania"
          value={formValues.Nowa_nazwa_zadania}
          onChange={handleChange}
          placeholder="Nowa Nazwa Zadania"
          className={inputStyle}
        />
      </div>
      <div className="mb-2">
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? 'Edytowanie...' : 'Edytuj Zadanie'}
      </button>
    </form>
  );
};
