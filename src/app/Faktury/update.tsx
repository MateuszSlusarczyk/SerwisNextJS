...
export const FakturyUpdate = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setFormValues({  nazwa: "", nowa_nazwa:"", id_projektu: "", wartość_netto: ""});

    try {
      const res = await fetch("/api/FakturyUpdate", {
        method: "POST",
    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="mb-3">
        <input
          ...
          type="text"
          name="nowa_nazwa"
          value={formValues.nowa_nazwa}
          onChange={handleChange}
          placeholder="Nowa nazwa Faktury"
          className={`${inputStyle}`}
        />
      </div>
      <div className="mb-3">
        <input
          ...
          onChange={handleChange}
          placeholder="Identifikator projektu"
          className={`${inputStyle}`}
        />
      </div>
      <div className="mb-3">
