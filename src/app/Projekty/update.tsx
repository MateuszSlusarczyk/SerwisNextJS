...
export const ProjektyUpdate = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setFormValues({  id_klienta: "", nazwa_projektu: "",nowa_nazwa_projektu:"", data_rozpoczecia: "", planowana_data_zakonczenia: ""});

    try {
      const res = await fetch("/api/ProjektyUpdate", {
        method: "POST",
    ...
    <form onSubmit={onSubmit}>
      ...
      <div className="mb-3">
        <input
          required
          type="text"
          name="nowa_nazwa_projektu"
          value={formValues.nowa_nazwa_projektu}
          onChange={handleChange}
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? "Edytowanie Projektu..." : "Edytuj Projekt"}
      </button>
    </form>
  );
};
